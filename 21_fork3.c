...
void fork3() {
  ...
  if(-1 == fork()) {
    ec_debug_log(LOG_ERR, "Unsuccessful", NULL);
     ...
     ///parent
     ec_debug_log(LOG_INFO, "waiting for child process...", NULL);
     wait(&status);

   if(WIFEXITED(status)) {
     ec_debug_log(LOG_INFO, "child process ended normally, exit status = %d\n", WIFEXITED(status));
   }

   if(WIFSIGNALED(status)) {
     ec_debug_log(LOG_INFO, "child process terminated by signal %d\n", WTERMSIG(status));
   }
   }


     ///child
     ec_debug_log(LOG_INFO, "child running...pid is %d\n", getpid());
      sleep(5);
      ec_cleanup_and_exit;
