...
void timestamp_file() {
  ...
  if(-1 != stat("bar" , &sb)) {
    ec_debug_log(LOG_INFO, "last accessed: %s", ctime(&sb.st_atime));
    ec_debug_log(LOG_INFO, "last mofified: %s", ctime(&sb.st_mtime));
    ec_debug_log(LOG_INFO, "last change: %s", ctime(&sb.st_ctime));
  } else {

  ec_debug_log(LOG_INFO, "Unable to access the file", NULL);
  ec_cleanup_and_exit;
